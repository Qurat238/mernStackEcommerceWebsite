...
export const getProduct = (keyword="",currentPage=1,price=[0,2500000],category,ratings=0) => async(dispatch) => {
    try {
        ...
        let link = `${window.location.origin}/api/v1/products?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${ratings}`;

        if(category){
            link = `${window.location.origin}/api/v1/products?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&category=${category}&ratings[gte]=${ratings}`;
        }
...
export const getAdminProduct = () => async(dispatch) => {
    try {
        ...
        const { data } = await axios.get(`${window.location.origin}/api/v1/admin/products`);
        dispatch({
...
export const  createProduct = (productData) => async(dispatch) => {
    try{ 
        ...
        const {data} = await axios.post(`${window.location.origin}/api/v1/admin/product/new`, productData, config);
...
export const deleteProduct = (id) => async(dispatch) => {
    try{ 
        ...
        const {data} = await axios.delete(`${window.location.origin}/api/v1/admin/product/${id}`);
...
export const  updateProduct = (id,productData) => async(dispatch) => {
    try{ 
        ...
        const {data} = await axios.put(`${window.location.origin}/api/v1/admin/product/${id}`, productData, config);
...
export const  getAllReviews = (id) => async(dispatch) => {
    try{ 
        ...
        const {data} = await axios.get(`${window.location.origin}/api/v1/reviews?id=${id}`);
...
export const  deleteReviews = (reviewId, productId) => async(dispatch) => {
    try{ 
        ...
        const {data} = await axios.delete(`${window.location.origin}/api/v1/reviews?id=${reviewId}&productId=${productId}`);
